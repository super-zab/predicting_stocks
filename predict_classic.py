"""Classic stock predictor: single Random Forest on a handful of technical features.

This is the original baseline kept side-by-side with the advanced ensemble
(`predict.py`) so you can compare them in the UI.
"""
...
def train_and_predict(ticker: str, period: str = "5y", **_) -> dict:
    ...
    return {
        ...
        "direction_proba": (pred_returns > 0).astype(float),
        "direction_actual": (y_test > 0).astype(int),
        "metrics": {
            "mae": mae, "rmse": rmse, "r2": r2,
            "direction_accuracy": float(((pred_returns > 0) == (y_test > 0)).mean()),
            "ci_halfwidth_return": float(np.median(1.96 * pred_std)),
            "horizon": 1,
        ...
        "next_proba_up": float(next_return > 0),
        "forecast_dates": [next_date],
        "forecast_prices": [next_price],
        "forecast_lower": [last_close * (1 + next_return - 1.96 * next_std)],
        "forecast_upper": [last_close * (1 + next_return + 1.96 * next_std)],
        "per_model_test_returns": {"random_forest": pred_returns},
        "feature_importance": {"random_forest": dict(zip(FEATURES, model.feature_importances_.tolist()))},
    }
